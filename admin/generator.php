            ...
            foreach ($courses as $courseid) {
                ...
                for ($i = 1; $i <= $this->get('number_of_sections'); $i++) {
                    for ($j = 0; $j < $this->get('number_of_modules'); $j++) {
                        ...
                        $locallibfile = "$CFG->dirroot/mod/$moduledata->name/locallib.php";
                        if (file_exists($locallibfile)) {
                            if (!in_array($locallibfile, $libraries)) {
                                $this->verbose("Including library for $moduledata->name...");
                                $libraries[] = $locallibfile;
                        ...
                        switch ($moduledata->name) {
                            ...
                            case 'survey':
                                ...
                                break;
                            case 'workshop':
                                $module->instructauthorseditor = 1;
                                $module->instructreviewerseditor = 1;
                                $module->grade = 1;
                                $module->gradinggrade = 1;
                                break;
                            case 'wiki':
                                $module->intro = $description;
                                $module->summary = $description;
        ...
        if (in_array('forum', $this->modules_list) &&
            ...
            foreach ($modules['forum'] as $forum) {
                ...
                for ($i = 0; $i < $this->get('discussions_per_forum'); $i++) {
                    ...
                    $discussion->message = '<p>Nothing much to say, since this is just a test...</p>';
